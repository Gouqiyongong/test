export default class CanvasRun {
  ...
  // 初始化
  public async init({canvas,ctx,config,gain=1,noNeedOff=false,mounted=()=>{},created=()=>{}}) {
    let lastMoveX = 0;
    this.canvas = canvas;
    this.ctx = ctx;
    this.gain = gain;
  ...
  // 编译步骤
  private async compile (VDomTree) {
    this.classList = {};
    const defaultEvent = () => {};
    ...
    const openAttr = (parent) =>{
      if(!parent) return;
      openParentAttr.forEach(i =>{
        if(parent.whiteSpace === 'nowrap') parent[i] = this.computedWidth(parent)[i] || parent[i];
      });
    };
    const combination = async (arr = VDomTree,parent = null) => {
      if(parent) parent.childX = 0;
      for(let i = 0; i < arr.length; i++){
        // 注册class
        arr[i].class && this.analysisString(arr[i].class).forEach(element => addClass(element,arr[i]));
  ...
  private dispatchEvent(events: any,ex,ey,type='click') {
  ...
  // 事件储存，把层级高的事件放在上面
  private eventStoreArea (type,envent) {
  ...
  private drawFont({fontSize,color='#000000',lineHeight,maxLine,text,textAlign='start',overText,overTextColor,fontWeight,x,y,w,h}) {
    this.ctx.save();
    ...
    computedTextArr.forEach((item,index)=>{
      ...
      if(index === maxLine - 1 && Math.abs(w - item.tw) < fontSize) {
        if(overText) {
          text = text.substr(0,text.length - (2 + overText.length)) + ('...');
  ...
  // 粒子化
  private getParticle (VCANVASDOM) {
    this.drwaView(VCANVASDOM);
    const arr = [];
    let {x,y,w,h,particle} = VCANVASDOM;
    const grainSize = particle.size || 5;
  ...
  // 事件包裹器
  private eventWrap({self,x,y,w,h,click,touchstart,touchmove,touchend}) {
    x /= this.gain;
    y /= this.gain;
    w /= this.gain;
    h /= this.gain;
  ...
  private get getFile() {
    ...
    if(this.VDomTree && this.VDomTree.length > 0){
      const combination = (arr = this.VDomTree,parent = null) => {
        if(parent) parent.childX = 0;
        for(let i = 0; i < arr.length; i++){
  ...
  // 滑动边界
  private slideSide(type,file,max,min) {
    if(file[type] > max) {
      file[type] = max;
  ...
  private slideTop(child,parent) {
    this.translateSlide(child,parent)
  };
  private translateSlide(child,parent) {
    let timer: any = 0;
